...
**En una línea:** el MVP de software está completo salvo la **impresión real** (parte 2, con la impresora), la
**gestión de productos en pantalla** y los **respaldos**. Lo que falta después es sobre todo puesta en marcha:
Supabase de producción, instalación en el PC del local, datos reales y el piloto con el cuaderno.

## PRs abiertos (integrar en este orden)

| PR | Contenido |
|---|---|
| ReizoVC/el-apurimeno#12 | Pantalla "Espejo en la nube" del Dashboard |
| ReizoVC/el-apurimeno#13 | `apps/owner` (resumen remoto con TOTP), migración `aal2`, paquete `@apurimeno/formato` |
| ReizoVC/el-apurimeno#14 | Esta revisión: auditoría, `CLAUDE.md`, README raíz y este documento |

Cada uno parte del anterior: hasta que se integra el previo, su diff muestra también esos commits.

## Completo y probado

| Parte | Qué hace | Cómo se probó |
|---|---|---|
| `packages/contracts` | Tipos y esquemas de todo el sistema; 22 decisiones de interpretación del SRS documentadas | 43 pruebas |
| `packages/domain` | Reglas de negocio: tiempos y cortesía, precios (lista, especial, ajuste), horas adicionales, caja y arqueo, tienda y stock, anulaciones, permisos, reportes, comprobante, resumen del espejo | 223 pruebas, incluida la tabla de casos del SRS |
| `packages/formato` | Dinero y fechas de Lima, compartidos por todas las pantallas | 8 pruebas |
| `apps/server` | API local: sesiones y permisos, turnos, ingresos, horas adicionales, salidas, tienda, anulación con código de autorización, limpieza, administración, configuración, auditoría, reportes, idempotencia de cobros, cola de impresión (parte 1) y sincronización con el espejo | 161 pruebas contra SQLite real; pruebas de concurrencia |
...
| `apps/web` (Dashboard) | Vista del día, reportes, habitaciones, clientes y precios especiales, usuarios y rangos, configuración, métodos de pago, auditoría, turnos abiertos y cierre forzado, códigos de anulación, reimpresión, espejo en la nube | De punta a punta como administrador contra el servidor real, con cifras calculadas a mano |
...
`pnpm check-types`, `pnpm lint` y `pnpm test` pasan en todo el monorepo (435 pruebas). Todas las apps compilan.
...
2. **Productos e inventario en pantalla.** El servidor ya permite crear categorías y productos, editarlos y
   reponer stock (RF-53, CU-29), pero **no hay pantalla** para eso: hoy el catálogo de la tienda solo se carga por
   la API. Hace falta una sección "Productos" en el Dashboard antes del piloto, o la tienda del POS queda vacía.
   (`apps/store-catalog` es otra cosa: una maqueta que se decidió no conectar todavía.)
3. **Respaldos** (Planos §14.3): no están implementados. El plan pide una copia consistente de la base cada
   15 minutos en el mismo equipo y una copia diaria fuera de él, y probar una restauración completa antes de
   producción. El espejo en la nube **no** es un respaldo: solo tiene totales.

### Puesta en marcha

4. **Aplicar la migración `20260926090000_lector_requiere_aal2.sql` en `apurimeno-prueba`** (SQL Editor; pasos
   en `supabase/README.md`). Hasta entonces la base no exige TOTP a las lectoras: la prueba confirmó que con solo
   la contraseña se puede leer por la API.
5. **Proyecto de Supabase de producción**, siguiendo `supabase/README.md`: aplicar las dos migraciones en orden,
...
6. **Publicar `apps/owner` en Cloudflare Pages** con las credenciales de producción (pasos exactos en
   `apps/owner/README.md`).
7. **Instalación en el PC del local:**
   ...
   - El instalador del POS (Tauri) compilado en un equipo con Visual Studio "Desarrollo para el escritorio con
     C++".
8. **Datos reales:** habitaciones y precios (la semilla trae 17 de ejemplo), métodos de pago, productos, las
   cuentas del personal con sus rangos, y el nombre, la dirección y la leyenda del comprobante.
9. **Piloto en paralelo con el cuaderno** (E4 de los Planos): 1 a 2 semanas usando ambos, con capacitación al
...
| Respaldo externo (§14.3) | No implementado | ¿Dónde va la copia diaria: un disco USB, una carpeta en la nube (Google Drive, OneDrive)? Define cómo se implementa |
| Pantalla de productos | No existe | Confirmar que se construye antes del piloto (recomendado) |
...
Ya resueltas y registradas (no requieren nada): comentario obligatorio con cualquier diferencia de arqueo
